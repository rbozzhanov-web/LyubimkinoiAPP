  ...
  return <>
    ...
    <IOSDialog visible={open} onClose={() => setOpen(false)} style={[styles.forecastPopup, { backgroundColor: palette.surfaceStrong, borderColor: palette.line }]}>
      ...
      {forecast && forecast.length > 0
        ? <Text numberOfLines={1} style={[styles.meta, styles.forecastLine, { color: palette.text }]}>
            {forecast.map((day) => `${forecastDayLabel(day.date)} ${weatherIcon(day.weatherCode, true).icon} ${day.tempMax}°/${day.tempMin}°`).join('  ·  ')}
          </Text>
...
const styles = StyleSheet.create({
  ...
  forecastPopup: { width: '88%', maxWidth: 340, borderWidth: 1, borderRadius: 22, padding: 18 }, forecastLine: { marginTop: 10, fontWeight: '600', fontVariant: ['tabular-nums'] },
