...
import type { RosterAbsence, RosterDuty, RosterSector } from './duties';
...
/**
 * Converts the structured response from AIMS eCrew CrewSchedule/SchedulerEvents
 * into the same roster model used by the existing PDF importer.
 *
 * This adapter deliberately does not handle authentication, cookies or CSRF tokens.
 * It accepts only the already-decoded SchedulerEvents JSON payload.
 */
