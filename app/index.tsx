...
export default function IndexScreen() {
  ...
  const importRoster = async () => {
    ...
    try {
      ...
      setRosters(next); setActiveMonth(parsed.period.start); setSelectedDuty(undefined); setTab('Roster');
  ...
  const changeMonth = (direction: -1 | 1) => {
    ...
    if (next) { setActiveMonth(next.period.start); setSelectedDuty(undefined); }
  };
  ...
  const submitCode = () => {
    ...
    setLovedMode(true); setUnlockOpen(false); setUnlockCode(''); setUnlockError(false);
  };

  return <SafeAreaView style={[styles.safe,{backgroundColor:palette.background}]} edges={['top','bottom']}>
    <View style={styles.app}>
      <View style={styles.header}><View><Text style={[styles.brand,{color:palette.text}]}>KhaVair</Text><Text style={[styles.kicker,{color:palette.muted}]}>CABIN CREW COMPANION</Text></View><Pressable onPress={requestLovedMode} style={[styles.modeButton,{backgroundColor:palette.surface}]}><Text style={styles.modeGlyph}>{lovedMode?'🌹':'♡'}</Text></Pressable></View>
...
function Home({roster,duties,rosters,cumulativeBlock,palette,onImport,importing}:{roster?:ParsedAirAstanaRoster;duties:Duty[];rosters:ParsedAirAstanaRoster[];cumulativeBlock:number;palette:Palette;onImport:()=>void;importing:boolean}) {
  ...
  return <View style={styles.screen}><Text style={[styles.label,{color:palette.muted}]}>LATEST IMPORT · {rosterMonthLabel(roster)}</Text><Card palette={palette}><View style={styles.routeRow}><Text style={[styles.airport,{color:palette.text}]}>{first.departure}</Text><Text style={[styles.routeArrow,{color:palette.muted}]}>→</Text><Text style={[styles.airport,{color:palette.text}]}>{last.arrival}</Text></View><Text style={[styles.meta,{color:palette.muted}]}>Report {duty.reportTime} · Release {duty.releaseTime} · {duty.sectors.length} sector{duty.sectors.length===1?'':'s'}</Text></Card><View style={styles.summaryRow}><Summary title="THIS MONTH" value={formatMinutes(roster.totals.blockMinutes)} detail={`${operatingCount(roster)} operating sectors`} palette={palette}/><Summary title="CUMULATIVE" value={formatMinutes(cumulativeBlock)} detail={`${rosters.length} month${rosters.length===1?'':'s'} imported`} palette={palette}/></View><Privacy palette={palette}/></View>;
}
...
function MoneyScreen({roster,stays,palette}:{roster?:ParsedAirAstanaRoster;stays:ReturnType<typeof detectStationStays>;palette:Palette}) {
  if(!roster)return <View style={styles.screen}><Text style={[styles.sectionTitle,{color:palette.text}]}>Money</Text><Card palette={palette}><Text style={[styles.meta,{color:palette.muted}]}>Import a roster first.</Text></Card></View>;
  const candidates=stays.filter((stay)=>stay.durationMinutes>120);
  return <View style={styles.screen}><Text style={[styles.sectionTitle,{color:palette.text}]}>Money</Text><Card palette={palette}><Text style={[styles.label,{color:palette.muted}]}>PER DIEM · {rosterMonthLabel(roster)}</Text><Text style={[styles.bigValue,{color:palette.text}]}>—</Text><Text style={[styles.meta,{color:palette.muted}]}>Station stays are detected. Payment units will be enabled after the UTC counting rule is confirmed.</Text></Card><View style={[styles.innerWindow,{backgroundColor:palette.surface,borderColor:palette.line}]}><FlatList data={candidates} keyExtractor={(item)=>`${item.arrivalLocal}-${item.station}`} contentContainerStyle={styles.listContent} ListEmptyComponent={<Text style={[styles.meta,{color:palette.muted}]}>No station stay longer than 2 hours detected.</Text>} renderItem={({item})=><View style={[styles.stayCard,{backgroundColor:palette.surfaceStrong,borderColor:palette.line}]}><View style={styles.sectorHeader}><Text style={[styles.stayStation,{color:palette.text}]}>{item.station}</Text><Text style={[styles.stayDuration,{color:palette.accent}]}>{formatStayDuration(item.durationMinutes)}</Text></View><Text style={[styles.meta,{color:palette.muted}]}>KC{item.arrivalFlight} → KC{item.departureFlight}{item.crossMonth?' · crosses month':''}</Text></View>}/></View></View>;
}

function MoreScreen({rosters,palette,onErase}:{rosters:ParsedAirAstanaRoster[];palette:Palette;onErase:()=>void}) {return <View style={styles.screen}><Text style={[styles.sectionTitle,{color:palette.text}]}>More</Text><Card palette={palette}><Text style={[styles.cardTitle,{color:palette.text}]}>Profile</Text><Text style={[styles.meta,{color:palette.muted}]}>Contract position · {DEFAULT_PROFILE.contractRank}</Text><Text style={[styles.meta,{color:palette.muted}]}>Roster position is display-only and never changes pay.</Text></Card><Card palette={palette}><Text style={[styles.cardTitle,{color:palette.text}]}>Local roster library</Text><Text style={[styles.meta,{color:palette.muted}]}>{rosters.length?rosters.map(rosterMonthLabel).join(' · '):'No months imported'}</Text></Card>{rosters.length>0&&<Pressable onPress={onErase} style={[styles.secondaryButton,{borderColor:palette.line}]}><Text style={[styles.secondaryText,{color:palette.text}]}>Erase all roster data</Text></Pressable>}</View>}

function DutyDetail({duty,palette}:{duty:Duty;palette:Palette}) {return <View style={[styles.detailWindow,{backgroundColor:palette.surfaceStrong,borderColor:palette.line}]}><Text style={[styles.cardTitle,{color:palette.text}]}>{duty.dateLabel}</Text><FlatList data={duty.sectors} keyExtractor={(item)=>item.id} contentContainerStyle={{paddingBottom:8}} renderItem={({item})=><View style={[styles.sectorBlock,{borderTopColor:palette.line}]}><View style={styles.sectorHeader}><Text style={[styles.sectorRoute,{color:palette.text}]}>{item.departure} → {item.arrival}</Text><Text style={[styles.meta,{color:palette.muted}]}>{item.flightNumber}{item.deadhead?' · DHC':''}</Text></View><Text style={[styles.meta,{color:palette.muted}]}>{item.departureTime} – {item.arrivalTime}</Text><Text style={[styles.flyingWith,{color:palette.accent}]}>Flying with · {item.crew.length}</Text>{item.crew.map((member)=><View key={member.id} style={styles.crewRow}><View style={[styles.avatar,{backgroundColor:palette.accentSoft}]}><Text style={[styles.avatarText,{color:palette.accent}]}>{member.name[0]}</Text></View><View style={{flex:1}}><Text style={[styles.crewName,{color:palette.text}]}>{member.name}</Text><Text style={[styles.meta,{color:palette.muted}]}>{member.position}</Text></View></View>)}</View>}/></View>}
