...
import { parseAimsCrewScheduleArchive } from './aimsCrewScheduleArchive';
...
export async function parseRosterData(data: ArrayBuffer, name = ''): Promise<ParsedAirAstanaRoster> {
  ...
  if (/CrewSchedule|initialResult|\/eCrew\/CrewSchedule/i.test(text)) return parseAimsCrewScheduleArchive(text);
  ...
  throw new Error(isWebArchive
    ...
    : 'Unsupported roster file. On iPhone: AIMS Crew Schedule → Share → Options → Web Archive → Save to Files. Then return to KhaVair → AIMS → Import Web Archive.');
}

/**
 * iPhone Safari saves Web Archive files as a binary property list. The main HTML resource is
 * embedded as raw bytes, so no plist execution/parsing is needed: decoding the local file
 * exposes the same inert HTML source consumed by the CrewSchedule parser. Detect the page
 * charset first so names/addresses survive.
 */
