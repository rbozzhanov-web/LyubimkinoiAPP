...
export function readRoster(columns: DayColumn[], periodStart: string, periodEnd: string): RosterReading {
  ...
  for (const column of columns) {
    ...
    while (i < cells.length) {
      ...
      if (cell === DELAY_LABEL) {
        i += 1; if (i < cells.length && isPlainTime(cells[i])) i += 1; continue;
      }
      ...
      // Ground duty such as MED1 08:00 17:00. Kept as a duty so it doesn't poison grid parsing,
      // but it is not counted as a flight sector.
      ...
      // Indicators such as M,R are metadata, not roster content.
